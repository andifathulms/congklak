...
import { derivePhase, wilayahDi, type Phase } from '@/lib/phase'
...
describe('derivePhase', () => {
  it('siap sebelum langkah pertama', () => {
    expect(derivePhase({ movesPlayed: 0, status: 'berjalan', busy: false })).toBe('siap')
  })

  it('main saat langkah pertama sedang ditabur — movesPlayed masih 0', () => {
    expect(derivePhase({ movesPlayed: 0, status: 'berjalan', busy: true })).toBe('main')
  })

  it('setelah begitu langkah selesai ditabur dan permainan berlanjut', () => {
    expect(derivePhase({ movesPlayed: 1, status: 'berjalan', busy: false })).toBe('setelah')
  })

  it('main lagi begitu langkah berikutnya mulai ditabur', () => {
    expect(derivePhase({ movesPlayed: 1, status: 'berjalan', busy: true })).toBe('main')
  })

  it('selesai begitu status berjalan menjadi selesai dan animasinya sudah berhenti', () => {
    expect(derivePhase({ movesPlayed: 8, status: 'selesai', busy: false })).toBe('selesai')
  })
  ...
  it('busy menang atas status: selagi bingkai terakhir masih diputar, tetap main', () => {
    ...
    expect(derivePhase({ movesPlayed: 7, status: 'selesai', busy: true })).toBe('main')
  })
})

describe('derivePhase — permainan acak yang direkam', () => {
  ...
  for (let seed = 1; seed <= JUMLAH_PERMAINAN; seed++) {
    it(`permainan #${seed}: fase mengikuti status GameState per giliran`, () => {
      ...
      expect(derivePhase({ movesPlayed, status: state.status, busy: false })).toBe('siap')
      ...
      while (state.status === 'berjalan') {
        ...
        expect(derivePhase({ movesPlayed, status: state.status, busy: true })).toBe('main')
        ...
        const fase = derivePhase({ movesPlayed, status: state.status, busy: false })
      ...
      // derivePhase tidak pernah teruji oleh jalur ini.
